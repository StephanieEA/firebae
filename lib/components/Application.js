...
export default class Application extends Component {
  ...
  submission() {
    const itemValue = this.state.draftItemValue
    firebase.database().ref(`users/${this.state.user.uid}/${this.state.draftBudgetItem}`).push({
      // user: pick(this.state.user, 'displayName', 'email', 'uid', 'photoURL', 'contactID'),
      // monthlyIncome: this.state.draftMonthly,
      // createdAt: Date.now(),
      itemValue
    })
    this.setState({ draftBudgetItem: '', draftItemValue: '' })
  }
